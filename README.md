...
Before proceeding, make sure that the following files are configured:

- `config.json` (location: `client/public/config/config.json`)
- Use `client/config/config.example.json` as a reference when creating or updating your configuration.
- Make sure `client/public/assets/logos` is populated with the correct `.svg` files.
  You can also use external URLs if your images are hosted elsewhere.
...
The runtime configuration file (`client/public/config/config.json`) is designed to be reusable across Beacon Template UI releases.
...
As long as your existing `config.json` matches the configuration expected by the new release, it can be reused without modification. If new fields are introduced in future versions, compare your configuration with `client/config/config.example.json` and extend it as needed.
